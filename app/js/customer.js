...
class CustomerDashboard {
  ...
  async buildSubscriptionPrefill() {
    try {
      const user = await this.api.getUser();
      if (!user) return null;
      const meta = user.user_metadata || {};
      let prenom = meta.prenom || '';
      let nom = meta.nom || '';
      if (!prenom && meta.name) {
      ...
      const policies = await this.api.getMyPolicies() || [];
      const latest = [...policies].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];
      if (latest) {
        if (!phone && latest.customer_phone) phone = latest.customer_phone;
        if (!address && latest.address) address = latest.address;
      }

      const fullName = [prenom, nom].filter((v) => v && v.trim()).join(' ').trim();
      return {
        email: user.email,
  ...
  async newSubscription() {
    ...
    const prefill = await this.buildSubscriptionPrefill();
  ...
  initSubscribeTunnel() {
    ...
    window.SURO_FORM.applyPrefill();
    window.SURO_FORM.applyLoggedInContactFromSession();
