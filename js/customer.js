...
class CustomerDashboard {
  ...
  init() {
    ...
    const allowedPages = ['dashboard', 'policies', 'claims', 'payments', 'profile'];
    this.navigateTo(allowedPages.includes(requestedPage) ? requestedPage : 'dashboard');
  }
  ...
  setupNavigation() {
    ...
    document.querySelectorAll('.link-view-all').forEach(link => {
      link.addEventListener('click', (e) => {
        e.preventDefault();
        this.navigateTo(link.getAttribute('href').replace('#', ''));
      });
    });

    window.addEventListener('popstate', () => {
      const page = window.location.hash.replace('#', '') || 'dashboard';
      if (['dashboard', 'policies', 'claims', 'payments', 'profile'].includes(page)) {
  ...
  async loadProfileHeader() {
    try {
      ...
      const name = String((user.user_metadata && user.user_metadata.name) || user.email || 'Client');
      document.getElementById('user-name').textContent = name;
      const firstName = name.includes('@') ? 'Client' : name.trim().split(/\s+/)[0];
      const welcomeName = document.getElementById('welcome-name');
      const avatar = document.getElementById('user-avatar');
      if (welcomeName) welcomeName.textContent = firstName;
      if (avatar) avatar.textContent = firstName.charAt(0).toUpperCase() || 'C';
    } catch (error) {
